...
class FullDPORecipeDistributed(FTRecipeInterface):
    ...
    def train(self) -> None:
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            # Track epoch start time for summary metrics
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                loss.backward()
                total_backward_passes += 1  # Only policy model gets gradients (reference is in eval mode)
                ...
                if (idx + 1) % self._gradient_accumulation_steps == 0:
                    ...
                    total_optimizer_steps += 1  # Track actual optimizer updates
                    ...
                    # Log per-step metrics and update progress bar together
                    if self._is_rank_zero:
                        ...
                        if self.global_step % self._log_every_n_steps == 0:
                            log_dict = {
                                ...
                                # Original per-GPU metric (keep for compatibility)
                                "tokens_per_second_per_gpu": num_tokens
                                / (time_per_step * self.world_size),
                                "tokens_per_second_total": tokens_per_sec_total,
                                "samples_per_second_total": samples_per_sec_total,
                                "effective_batch_size": effective_batch_size,
                                "forward_passes_completed": total_forward_passes,
